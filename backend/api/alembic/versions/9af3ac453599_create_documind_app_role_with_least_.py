...
down_revision: str | Sequence[str] | None = "6358bb32b8c2"
...
def upgrade() -> None:
    ...
    op.execute(
        "GRANT SELECT, INSERT, UPDATE, DELETE ON documents, tenants, chunks TO documind_app;"
    )
...
def downgrade() -> None:
    op.execute(
        "REVOKE SELECT, INSERT, UPDATE, DELETE ON documents, tenants, chunks FROM documind_app;"
    )
